...
function fakeQueue() {
  ...
  return { added, add: async (name: string, data: any) => { added.push({ name, data }); return { id: `job-${added.length}` } } }
}
...
describe('replayDeadLetter', () => {
  async function seedDLQ(payload: any) {
  ...
  it('re-enqueues the original payload verbatim and marks the item replayed', async () => {
    ...
    const result = await replayDeadLetter(String(dl._id), { ingestQueue: queue as unknown as Queue, tenantId })
  ...
  it('does not replay another tenant\'s DLQ item (returns null, enqueues nothing)', async () => {
    ...
    const queue = fakeQueue()
    const result = await replayDeadLetter(String(dl._id), { ingestQueue: queue as unknown as Queue, tenantId: new mongoose.Types.ObjectId() })
...
describe('backfillConnection', () => {
  it('re-enqueues one ingest job per raw_record for the connection', async () => {
    ...
    const result = await backfillConnection(connectionId, { ingestQueue: queue as unknown as Queue, tenantId })
  ...
  it('only backfills the requesting tenant\'s raw records', async () => {
    ...
    const queue = fakeQueue()
    const result = await backfillConnection(connectionId, { ingestQueue: queue as unknown as Queue, tenantId: new mongoose.Types.ObjectId() })
