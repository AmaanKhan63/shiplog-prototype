...
async function waitFor<T>(predicate: () => T | Promise<T>, { timeout = 10000, interval = 40 } = {}) {
...
describe.skipIf(!available)('failure -> DLQ -> replay -> no duplicate (live Redis)', () => {
  ...
  it('replays the dead-lettered record onto the SAME event row, count unchanged', async () => {
    ...
    failMode = null
    const result = await replayDeadLetter(dl._id as unknown as string, { ingestQueue, tenantId })
