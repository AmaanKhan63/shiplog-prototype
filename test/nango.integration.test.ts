...
async function waitFor<T>(pred: () => Promise<T>, { timeout = 15000, interval = 50 } = {}) {
...
describe.skipIf(!available)('Nango webhook -> records -> events (live Redis, fixture Nango)', () => {
  ...
  let nangoSyncQueue: ReturnType<typeof createNangoSyncQueue>
  let ingestWorker: ReturnType<typeof createIngestWorker>
  let syncWorker: ReturnType<typeof createNangoSyncWorker>
  ...
  function postWebhook(payloadObj: unknown) {
