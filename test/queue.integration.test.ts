...
async function waitFor<T>(predicate: () => Promise<T>, { timeout = 10000, interval = 40 } = {}) {
...
describe.skipIf(!available)('BullMQ ingest pipeline (live Redis)', () => {
  ...
  let dlqQueue: ReturnType<typeof createDlqQueue>
  ...
  it('processes a healthy record end-to-end into the event spine', async () => {
    ...
    await ingestQueue.add('ingest', { tenantId: tenantId as unknown as string, record: githubFixtures[0] })
  ...
  it('retries a transient failure 5 times then writes exactly one DLQ doc', async () => {
    const tenantId = new mongoose.Types.ObjectId()
    await ingestQueue.add('ingest', { tenantId: tenantId as unknown as string, record: githubFixtures[0], poison: 'transient' })
  ...
  it('sends a logical failure straight to the DLQ with no retry', async () => {
    const tenantId = new mongoose.Types.ObjectId()
    await ingestQueue.add('ingest', { tenantId: tenantId as unknown as string, record: githubFixtures[0], poison: 'logical' })
