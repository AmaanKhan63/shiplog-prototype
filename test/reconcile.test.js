...
function fakeQueue() {
  const added = []
  return { added, add: async (name, data, opts) => { added.push({ name, data, opts }); return { id: `j${added.length}` } } }
}
...
function fakeNango(pages) {
  ...
  return {
    ...
    async listRecords(args) {
      ...
      const page = pages[i++]
      if (page === 'THROW') { const e = new Error('Nango records API unavailable'); e.kind = 'transient'; throw e }
...
// of hanging.
function loopingNango(page, cap = 50) {
...
function failingIngestQueue(failOnNth) {
  let n = 0
  const added = []
  return { added, add: async (name, data) => { n += 1; if (n === failOnNth) throw new Error('ingest enqueue failed'); added.push({ name, data }); return { id: `j${n}` } } }
}

const issue = (id, number, cursor) => ({
...
const job = (data) => ({ data: { tenantId, connectionId, nangoConnectionId: 'nc-1', providerConfigKey: 'github', model: 'GithubIssue', ...data } })
