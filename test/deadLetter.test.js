...
describe('isTerminalFailure', () => {
  it('is terminal immediately for an UnrecoverableError (logical, no retry)', () => {
    expect(isTerminalFailure({ attemptsMade: 1, opts: { attempts: 5 } }, unrecoverable())).toBe(true)
  })

  it('is not terminal while transient attempts remain', () => {
    expect(isTerminalFailure({ attemptsMade: 2, opts: { attempts: 5 } }, transient())).toBe(false)
  })

  it('is terminal once transient attempts are exhausted', () => {
    expect(isTerminalFailure({ attemptsMade: 5, opts: { attempts: 5 } }, transient())).toBe(true)
  })
})
