...
import type { IngestJobData, JobView, ReconcileJobData } from '../queue/types.js'
 ...
 */
export function makeReconcileProcessor({ nango, ingestQueue }: { nango: NangoClientLike; ingestQueue: Queue<IngestJobData> }) {
  return async function reconcileProcessor(job: JobView<ReconcileJobData>): Promise<ReconcileResult> {
    ...
    let cursor: string | null | undefined = state?.cursor

    let fetched = 0
    let enqueued = 0
    let pageCursor: string | null | undefined = cursor
