...
async function waitFor(pred, { timeout = 30000, interval = 50 } = {}) {
...
async function main() {
  ...
  const nango = {
    ...
    async listRecords(args) {
      if (nangoDown) { const e = new Error('Nango records API unavailable (injected)'); e.kind = 'transient'; throw e }
  ...
  const wlog = { log: (m) => console.log('       ' + m), error: (m) => console.log('       ' + m) }
