...
const record = githubFixtures.find((r) => r._nango_metadata.model === 'GithubCommit') // immutable commit

const delay = (ms) => new Promise((res) => setTimeout(res, ms))
async function waitFor(pred, { timeout = 25000, interval = 50 } = {}) {
...
async function main() {
  ...
  const workerLogger = { log: (m) => console.log('       ' + m), error: (m) => console.log('       ' + m) }
